...
export function Terminal({ fileTree, onFileTreeUpdate }: TerminalProps) {
  ...
  const initializeTerminal = async (terminal: XTerm) => {
    try {
      ...
      terminal.writeln(
        "\x1b[32m✓\x1b[0m Connecting to development environment..."
      );
    ...
    try {
      switch (cmd) {
        case "help":
          ...
          terminal.writeln(
            "  \x1b[33mls\x1b[0m          - List files and directories"
          );
          ...
          terminal.writeln(
            "  \x1b[33mrm\x1b[0m          - Remove file/directory"
          );
          terminal.writeln(
            "  \x1b[33mnpm\x1b[0m         - Node package manager"
          );
          terminal.writeln(
            "  \x1b[33mgit\x1b[0m         - Git version control"
          );
  ...
  const handleMkdirCommand = async (terminal: XTerm, dirName?: string) => {
    ...
    const newPath =
      currentDirectory === "/"
        ? dirName
        : `${currentDirectory}/${dirName}`.replace("//", "/");
  ...
  const handleNpmCommand = async (terminal: XTerm, args: string[]) => {
    ...
    switch (subcommand) {
      ...
      case "start":
        ...
        await new Promise((resolve) => setTimeout(resolve, 1000));
        terminal.writeln(
          "\x1b[32m✓\x1b[0m Server running on http://localhost:3000"
        );
        break;
  ...
  const handleGitCommand = async (terminal: XTerm, args: string[]) => {
    ...
    switch (subcommand) {
      case "status":
        ...
        terminal.writeln(
          "  \x1b[31mmodified:   src/components/Header.js\x1b[0m"
        );
        break;
      ...
      case "add":
        ...
        if (files.includes(".")) {
          ...
          terminal.writeln(
            `\x1b[32m✓\x1b[0m Added ${files.join(", ")} to staging area`
          );
        } else {
      ...
      case "commit":
        ...
        const message =
          messageIndex !== -1 ? args[messageIndex + 1] : "Update files";
      ...
      case "push":
        ...
        terminal.writeln(
          "   \x1b[36mremote:\x1b[0m Resolving deltas: 100% (2/2), done."
        );
      ...
      case "init":
        terminal.writeln(
          "\x1b[32m✓\x1b[0m Initialized empty Git repository in .git/"
        );
        break;
      ...
      case "clone":
        ...
        terminal.writeln(
          `Cloning into '${repoUrl.split("/").pop()?.replace(".git", "")}'...`
        );
        await new Promise((resolve) => setTimeout(resolve, 2000));
      ...
      default:
        terminal.writeln(
          `\x1b[31m✗\x1b[0m git: '${subcommand}' is not a git command.`
        );
