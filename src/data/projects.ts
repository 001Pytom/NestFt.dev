...
// Existing projects from the main projects page
...
// Export all projects (existing + new beginner projects)
...
export const projectsByCategory = {
  frontend: existingProjects.filter(p => p.technologies.some(tech => ['HTML/CSS', 'JavaScript', 'React', 'Vue', 'Angular'].includes(tech)) && !p.technologies.some(tech => ['Node.js', 'Express', 'MongoDB'].includes(tech))),
  backend: backendBeginnerProjects,
  fullstack: [...existingProjects.filter(p => p.technologies.some(tech => ['Node.js', 'Express', 'MongoDB'].includes(tech))), ...fullstackBeginnerProjects]
}

// Export by difficulty
export const projectsByDifficulty = {
  easy: allProjects.filter(p => p.difficulty === 'Easy'),
